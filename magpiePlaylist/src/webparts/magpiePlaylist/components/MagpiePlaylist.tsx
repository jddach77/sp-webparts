...
export default class ReactSlideSwiper extends React.Component<
  IMagpiePlaylistProps,
  IMagpiePlaylistState
> {
  private uniqueId: number;

  constructor(props: IMagpiePlaylistProps) {
    super(props);
  ...
  public componentDidMount(): void {
    this.props.listService
      .getAll(this.props.listName)
      .then((result: Array<ListItem>) => {
  ...
  public render(): React.ReactElement<IMagpiePlaylistProps> {
    return (
      <div className={styles.reactSlideSwiper}>
        <div
          className={`swiper-container ${styles.container} container-${
            this.uniqueId
          }`}
        >
          <div className="swiper-wrapper">
            {this.state.listItems.length &&
              this.state.listItems.map((listItem, i) => {
                return (
                  <div className={`swiper-slide ${styles.slide}`} key={i}>
          ...
          {this.props.swiperOptions.enableNavigation && (
            <div className={`swiper-button-next next-${this.uniqueId}`} />
          )}
          {this.props.swiperOptions.enableNavigation && (
            <div className={`swiper-button-prev prev-${this.uniqueId}`} />
          )}

          {this.props.swiperOptions.enablePagination !== false && (
            <div className={`swiper-pagination pagination-${this.uniqueId}`} />
          )}
        </div>
      </div>
    );
