...
class WriteManifestsTask extends OdspGulpTask_1.default {
    constructor() {
        ...
        this._defaultLanguagesMapping = undefined;
    }
    loadSchema() {
        return node_core_library_1.JsonFile.load(path.join(__dirname, 'write-manifests.schema.json'));
    }
    executeTask(gulp, completeCallback) {
        ...
        const debugLocale = normalizeLocaleName_1.default(this.taskConfig.buildSingleLocale || constants_1.default.defaultLocale);
        this.logVerbose(`Using debug locale: ${debugLocale}`);
        ...
        this.properties.deployEntries.forEach((deployEntry) => {
            ...
            componentKeys.forEach((id) => {
                ...
                if (shouldProduceDebugManifests) {
                    let debugManifest = this._doComponentTypeSpecificFixup(true, bundleComponent.manifestData, bundleComponent.manifestPath, debugLocale);
                    ...
                    //  production build
                    this.buildConfig.production ? undefined : debugLocale, !this.buildConfig.production);
        ...
        merge(tasks).on('finish', () => {
            ...
            if (shouldProduceDebugManifests) {
                ...
                const cumulativeManifestProcessor = cumulativeManifestProcessor_1.default.initializeFromGulpTask(this, cumulativeManifestOptions);
        ...
        if (iconPathType) {
            switch (iconPathType) {
                ...
                case IconPathType.DataUrl:
                    this.logVerbose(`Found data:image icon URL "${iconPath}"`);
                    break;
    ...
    _getDefaultLangaugesMapping(normalizedLocaleName) {
        if (this._defaultLanguagesMapping === undefined) {
            this._defaultLanguagesMapping = {};
            this._defaultLanguagesMapping['ar-SA'] = ['ar', 'tzm-Latn', 'ku', 'syr'];
            this._defaultLanguagesMapping['cy-GB'] = ['cy'];
            this._defaultLanguagesMapping['da-DK'] = ['fo', 'kl'];
            this._defaultLanguagesMapping['de-DE'] = ['de', 'dsb', 'rm', 'hsb'];
            this._defaultLanguagesMapping['el-GR'] = ['el'];
            this._defaultLanguagesMapping['en-AU'] = ['en-SG', 'en-HK', 'en-MY', 'en-PH', 'en-TT', 'en-AZ', 'en-BH', 'en-BN',
                'en-ID', 'mi'];
            this._defaultLanguagesMapping['en-GB'] = ['sq', 'am', 'hy', 'mk', 'bs', 'my', 'dz', 'en-CY', 'en-EG', 'en-IL',
                'en-IS', 'en-JO', 'en-KE', 'en-KW', 'en-MK', 'en-MT', 'en-PK', 'en-QA', 'en-SA', 'en-LK', 'en-AE', 'en-VN',
                'is', 'km', 'kh', 'mt', 'fa', 'gd', 'sr-Cyrl-BA', 'sr-Latn-BA', 'sd', 'si', 'so', 'ti-ET', 'uz'];
            this._defaultLanguagesMapping['en-IN'] = ['as', 'bn-BD', 'bn-IN', 'gu', 'kn', 'kok', 'ml', 'mr', 'ne', 'or',
                'pa', 'sa', 'ta', 'te', 'ur'];
            this._defaultLanguagesMapping['en-US'] = ['bn', 'chr', 'dv', 'div', 'en', 'fil', 'haw', 'iu', 'lo', 'moh'];
            this._defaultLanguagesMapping['en-ZA'] = ['af', 'en-ZW', 'en-ZM', 'en-NG', 'en-OM', 'ha', 'ig', 'xh', 'zu',
                'rw,', 'sw', 'nqo', 'om', 'nso', 'st', 'tn', 'sn', 'ti', 'ts', 've', 'yo'];
            this._defaultLanguagesMapping['es-ES'] = ['gn', 'quz', 'es', 'ca-ES-valencia'];
            this._defaultLanguagesMapping['fi-FI'] = ['sms', 'se-FI', 'se-Latn-FI'];
            this._defaultLanguagesMapping['fr-FR'] = ['gsw', 'br', 'tzm-Tfng', 'co', 'fr', 'ff', 'lb', 'mg', 'oc', 'zgh',
                'wo'];
            this._defaultLanguagesMapping['hi-IN'] = ['hi'];
            this._defaultLanguagesMapping['id-ID'] = ['jv'];
            this._defaultLanguagesMapping['it-IT'] = ['it'];
            this._defaultLanguagesMapping['ms-MY'] = ['ms'];
            this._defaultLanguagesMapping['nb-NO'] = ['no', 'nb', 'nn', 'smj-NO', 'smj-Latn-NO', 'se-NO', 'se-Latn-NO',
                'sma-Latn', 'sma-NO'];
            this._defaultLanguagesMapping['nl-NL'] = ['nl', 'fy'];
            this._defaultLanguagesMapping['prs-AF'] = ['gbz', 'ps'];
            this._defaultLanguagesMapping['pt-PT'] = ['pt'];
            this._defaultLanguagesMapping['ro-RO'] = ['ro'];
            this._defaultLanguagesMapping['ru-RU'] = ['ru', 'ba', 'be', 'ky', 'mn', 'sah', 'tg', 'tt', 'tk'];
            this._defaultLanguagesMapping['sk-SK'] = ['sk'];
            this._defaultLanguagesMapping['sl-SI'] = ['sl'];
            this._defaultLanguagesMapping['sr-Cyrl-RS'] = ['sr-Cyrl'];
            this._defaultLanguagesMapping['sr-Latn-RS'] = ['sr'];
            this._defaultLanguagesMapping['sv-SE'] = ['smj', 'se', 'sv', 'sma-SE', 'sma-Latn-SE'];
            this._defaultLanguagesMapping['th-TH'] = ['th'];
            this._defaultLanguagesMapping['tr-TR'] = ['tr'];
            this._defaultLanguagesMapping['vi-VN'] = ['vi'];
            this._defaultLanguagesMapping['zh-CN'] = ['zh', 'mn-Mong', 'bo', 'ug', 'ii'];
            this._defaultLanguagesMapping['zh-TW'] = ['zh-HK', 'zh-CHT', 'zh-Hant', 'zh-MO'];
        }
        return this._defaultLanguagesMapping[normalizedLocaleName];
    }
            ...
            deployModules.localizedPathModules.forEach((module, id) => {
                ...
                for (const locale in oldPaths) {
                    if (oldPaths.hasOwnProperty(locale)) {
                        ...
                        const baseLanguageTranslations = this._getDefaultLangaugesMapping(normalizedLocale);
                        if (baseLanguageTranslations !== undefined) {
                            baseLanguageTranslations.forEach((translation) => {
                                module.paths[translation] = resolvePath(resourcePath);
                            });
                        }
                    }
                }
