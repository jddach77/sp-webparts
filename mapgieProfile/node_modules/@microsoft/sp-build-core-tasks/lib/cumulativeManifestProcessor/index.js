...
class CumulativeManifestProcessor {
    ...
    generateCumulativeManifest(gulp, debugManifests) {
        ...
        Object.keys(manifests).forEach(id => {
            if (!this._options.ignoreOutputManifestIds || this._options.ignoreOutputManifestIds.indexOf(id) === -1) {
                Object.keys(manifests[id]).forEach(version => manifestsArray.push(manifests[id][version]));
            }
        });
        // Write the manifests.json file
        result.push(FileUtils_1.writeStringToFile(JSON.stringify(manifestsArray, undefined, 2), `${CUMULATIVE_MANIFEST_FILENAME}.json`).pipe(gulp.dest(this._options.tempFolderName)));
        // Write the manifests.js file
        result.push(gulp.src(this.manifestsFilePath).pipe(webpackStream(this.getManifestsFileWebpackConfig(manifestsArray), Webpack, (error, stats) => {
            if (error) {
                this._options.logError(`Error generating cumulative manifest: ${error.toString()}`);
            }
        })).pipe(gulp.dest(this._options.tempFolderName)));
        return result;
    }
    ...
    discoverManifests(packagePath, forceSearchMode, options = {}) {
        ...
        const tempPath = path.join(resolvedPackagePath, this._options.tempFolderName || 'temp');
        const targetFolderPath = path.join(resolvedPackagePath, optionsToUse.projectTargetFolder);
        ...
        if (!linkedProjectManifests) {
            ...
            try {
                fs.readdirSync(targetFolderPath).forEach((targetFilename) => {
            ...
            for (const manifest of linkedProjectManifests) {
                if (manifestsToFilter.indexOf(manifest.id) === -1) {
                    ...
                    if (componentManifest.version) {
                        ...
                        throw new Error(`Invalid manifest with id "${id}" in ${resolvedPackagePath}.`);
                    }
                }
            }
        }
