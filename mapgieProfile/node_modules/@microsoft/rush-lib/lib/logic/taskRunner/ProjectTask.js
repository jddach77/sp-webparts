...
class ProjectTask {
    ...
    execute(writer) {
        try {
            const taskCommand = this._getScriptToRun();
            const deps = this._getPackageDependencies(taskCommand, writer);
    ...
    _getPackageDependencies(taskCommand, writer) {
        let deps = undefined;
    ...
    _executeTask(taskCommand, writer, currentPackageDeps) {
        try {
            ...
            if (node_core_library_1.FileSystem.exists(currentDepsPath)) {
                try {
                    ...
                    writer.writeLine(`Warning: error parsing ${RushConstants_1.RushConstants.packageDepsFilename}: ${e}. Ignoring and ` +
            ...
            if (isPackageUnchanged && this.isIncrementalBuildAllowed) {
                ...
                // Run the task
                const normalizedTaskCommand = process.platform === 'win32'
                    ? convertSlashesForWindows(taskCommand)
                    : taskCommand;
                writer.writeLine(normalizedTaskCommand);
                const task = Utilities_1.Utilities.executeLifecycleCommandAsync(normalizedTaskCommand, projectFolder, this._rushConfiguration.commonTempFolder, true);
