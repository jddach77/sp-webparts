...
class InstallManager {
    ...
    doInstall() {
        return Promise.resolve().then(() => {
            ...
            ApprovedPackagesChecker_1.ApprovedPackagesChecker.rewriteConfigFiles(this._rushConfiguration);
            ...
            const hookDestination = path.join(this._rushConfiguration.rushJsonFolder, ".git" /* Git */, 'hooks');
            if (node_core_library_1.FileSystem.exists(hookSource)) {
                ...
                if (hookFilenames.length > 0) {
                    ...
                    for (const filename of filteredHookFilenames) {
                        ...
                        node_core_library_1.FileSystem.changePosixModeBits(path.join(hookDestination, filename), 64 /* UserExecute */);
                    }
            ...
            return this.ensureLocalPackageManager()
                ...
                return this._installCommonModules(Object.assign({ shrinkwrapIsUpToDate,
    ...
    _createTempModulesAndCheckShrinkwrap(options) {
        ...
        for (const rushProject of sortedRushProjects) {
            ...
            for (const [packageName, packageVersion] of tempDependencies.entries()) {
                ...
                if (shrinkwrapFile) {
                    if (!shrinkwrapFile.tryEnsureCompatibleDependency(packageName, packageVersion, rushProject.tempProjectName)) {
