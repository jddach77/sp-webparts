...
class PublishUtilities {
            ...
            if (allChanges.hasOwnProperty(packageName)) {
                ...
                if (skipVersionBump) {
                    ...
                    change.newVersion = (change.changeType >= ChangeManagement_1.ChangeType.patch) ?
                        ...
                        (change.changeType === ChangeManagement_1.ChangeType.hotfix ? change.newVersion : pkg.version);
                }
                if (deps) {
    ...
    static execCommand(shouldExecute, command, args = [], workingDirectory = process.cwd(), environment) {
        ...
        if (shouldExecute) {
            Utilities_1.Utilities.executeCommand(command, args, workingDirectory, environment, false, true);
        }
    }
    ...
    static _writePackageChanges(change, allChanges, allPackages, rushConfiguration, shouldCommit, prereleaseToken, projectsToExclude) {
        ...
        if (shouldCommit) {
            node_core_library_1.JsonFile.save(pkg, packagePath);
        }
        return pkg;
    }
    ...
    static _addChange(change, allChanges, allPackages, rushConfiguration, prereleaseToken, projectsToExclude) {
        ...
        if (skipVersionBump) {
            ...
            currentChange.newRangeDependency = change.changeType === ChangeManagement_1.ChangeType.hotfix ?
