...
class RushConfiguration {
    ...
    constructor(rushConfigurationJson, rushJsonFilename) {
        ...
        if (this._packageManager === 'npm') {
            ...
            this._tempShrinkwrapFilename = path.join(this._commonTempFolder, RushConstants_1.RushConstants.yarnShrinkwrapFilename);
            this._packageManagerToolVersion = rushConfigurationJson.yarnVersion;
            this._packageManagerToolFilename = path.resolve(path.join(this._commonTempFolder, 'yarn-local', 'node_modules', '.bin', 'yarn'));
        }
    ...
    static loadFromConfigurationFile(rushJsonFilename) {
        const resolvedRushJsonFilename = path.resolve(rushJsonFilename);
        ...
        for (const filename of node_core_library_1.FileSystem.readFolder(commonRushConfigFolder)) {
            ...
            switch (packageManager) {
                ...
                case 'pnpm':
                    ...
                    break;
                case 'yarn':
                    knownSet.add(RushConstants_1.RushConstants.yarnShrinkwrapFilename.toUpperCase());
                    break;
            }
    ...
    getCommittedShrinkwrapFilename(variant) {
        ...
        if (this.packageManager === 'pnpm') {
            return path.join(variantConfigFolderPath, RushConstants_1.RushConstants.pnpmShrinkwrapFilename);
        }
        else if (this.packageManager === 'npm') {
            return path.join(variantConfigFolderPath, RushConstants_1.RushConstants.npmShrinkwrapFilename);
        }
        else if (this.packageManager === 'yarn') {
            return path.join(variantConfigFolderPath, RushConstants_1.RushConstants.yarnShrinkwrapFilename);
        }
        else {
