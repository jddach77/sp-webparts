...
class Utilities {
    ...
    static isFileTimestampCurrent(outputFilename, inputFilenames) {
        if (!node_core_library_1.FileSystem.exists(outputFilename)) {
            return false;
        }
        const outputStats = node_core_library_1.FileSystem.getStatistics(outputFilename);
    ...
    static wrapWords(text, maxLineLength, indent) {
        ...
        const wrap = wordwrap.soft(indent, maxLineLength);
        return wrap(text);
    }
    /**
    ...
    static executeLifecycleCommand(command, options) {
        ...
        const environment = Utilities._createEnvironmentForRushCommand(options.initCwd);
    ...
    static installPackageInDirectory(options) {
        ...
        Utilities.executeCommandWithRetry(options.maxInstallAttempts, 'npm', ['install'], directory, Utilities._createEnvironmentForRushCommand(''), options.suppressOutput);
    }
     ...
     */
    static syncNpmrc(sourceNpmrcFolder, targetNpmrcFolder) {
