...
class Npm {
    static publishedVersions(packageName, cwd, env) {
        const versions = [];
        try {
            const packageTime = Utilities_1.Utilities.executeCommandAndCaptureOutput('npm', `view ${packageName} time --json`.split(' '), cwd, env, true);
            if (packageTime && packageTime !== '') {
                ...
                const packageVersions = Utilities_1.Utilities.executeCommandAndCaptureOutput('npm', `view ${packageName} versions --json`.split(' '), cwd, env, true);
